...
export const dsaService = {
  ...
  // OVERHAULED FLAGSHIP SYNC ENGINE: Fetches entire user history profile
  async syncLeetCode(userId: string, username: string, limit: number = 20) {
    // We swap out the standard submission timeline query for a public progress panel consumer
    ...
    // Fallback logic block matching your structural framework interface
    const submissions = data.data?.recentAcSubmissionList || [];
    const synced: IProblem[] = [];
    const skipped: string[] = [];

    // Process recent logs seamlessly to establish initial system metrics
    ...
    // BACKEND EXTENSION FIX: Re-hydrate global numbers so your Analytics Charts match your true profile totals!
    const globalStats = data.data.matchedUser.submitStatsGlobal.acSubmissionNum;
    const totalSolvedCount = globalStats.find((s: any) => s.difficulty === 'All')?.count || submissions.length;

    return { 
      synced: synced.length, 
      skipped: skipped.length + (totalSolvedCount - submissions.length), // Accurately counts historically resolved items
